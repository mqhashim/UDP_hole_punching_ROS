...
public class UdpServer {
    ...
    public void start() {
        ...
        new Thread(new Runnable() {
            public void run() {
                if (_port == 11411) {
                    InetAddress AWS_addr;
                    String hostname = "3.12.146.3";
                    int port = 32145;
                    InetSocketAddress addr = new InetSocketAddress(hostname, port);
                    byte[] bytes = new byte[] {82,2}; //"R".getBytes();
                    while (_socket.isBound() && !_socket.isClosed()) {
                        try {
                            DatagramPacket to_be_sent = new DatagramPacket(bytes, 2, addr);
                            _socket.send(to_be_sent);
                            logger.log(Level.ALL, "Success AWS");
                            Thread.sleep(2000);
                        } catch (IOException e) {
                            logger.log(Level.WARNING, "Failed to send AWS ack, exiting thread", e);
